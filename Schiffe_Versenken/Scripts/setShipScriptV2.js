...
$(document).ready(function(){
        ...
        $(".btn_battleship").mousedown(function() {btnEvents(countBattleship, 2, 5,"battleship" )}); //überprüfen, ob shipClass funktioniert (wenn nicht, dann if-else)
        $(".btn_battleship").mousedown(function() {btnEvents(countCruiser, 2, 4, "cruiser")}); //überprüfen, ob shipClass funktioniert
        $(".btn_battleship").mousedown(function() {btnEvents(countDestroyer, 3, 3, "destroyer")});
        $(".btn_battleship").mousedown(function() {btnEvents(countSubmarine, 4, 2, "submarine")});
        $(".btn_battleship").mousedown(function() {btnEvents(countDinghy, 5, 1, "dinghy")});
    ...
    $(document).on('mouseenter', '.fieldPoint', function() {
        ...
        updateShipsMouseenter(); //Übergebene Werte anpassen
    });
...
function shipColissionVertical(fromRow, toRow, thisCellNb, shipClass, row) {
    ...
    for (var h = fromRow; h < toRow; h++) {
        ...
        if (h > 15) { h = 15; }
         ...
         //  console.log("In der Celle bin ich: "+thisCell);
         var allOfClass= thisCell.prop('className').split(" ");
         var ifShip = allOfClass[1]; //übreprüfen, ob index richtig it.
       
        //  var ifShip = thisCell.prop('className').substring(11);
...
function updateShipsClick(isHorizontal, cellNb, rowNb){ //isHorizontal mit Variable für jeweiliges Schiff angeben
    if (isHorizontal) { 
        ...
        //siehe Kommentar bei else... gleicher Fall nur in grün
        ...
        //Soll das Schiff 'vertical' gesetzt werden, wenn das schiff NICHT Horizonztal(=Vertical) ist?
        //Sollte das Schiff rein von der Logik her Horizontal gesetzt werden, wenn es Vertical ist?! 
...
function updateShipsMouseenter(){
    var cellName = $(this).prop('id');
    var cellNb = parseInt(cellName.substring(4, 6));
    if (isHorizontal === false) { //vergleiche mit updateShipClick und entscheide, ob true oder false
        if (shipSelection === true) {
        ...
        if (shipSelection === true) { //gesamte If Abfrage ändern bzw. einheitlich gestalten
            ...
            setShipVertical(rowNb, shipEnd, cellNb, shipClass);
...
function setShipVertical(rowNb, shipEnd, cellNb, shipClass){//Sind an den Stellen wo die Funktion aufgerufen wird die Variabeln definiert?
       
    if(shipColissionVertical(rowNb,shipEnd,cellNb ,shipClass) == true){
        row = $(this).parent();
        $(".fieldPoint").removeClass(shipClass);
